...
class _Bottleneck(tf.keras.Model):
    def __init__(self, filters, block, downsampling=False, stride=1, **kwargs):
        ...
        # eg 64 to 256 to 64
        self.conv2a = tf.keras.layers.Conv2D(filters1, (1, 1), strides=(stride, stride),
                                             kernel_initializer='he_normal', name=conv_name_base + '2a')
        self.bn2a = tf.keras.layers.BatchNormalization(name=bn_name_base + '2a')

        self.conv2b = tf.keras.layers.Conv2D(filters2, (3, 3), padding='same',
                                             kernel_initializer='he_normal', name=conv_name_base + '2b')
        self.bn2b = tf.keras.layers.BatchNormalization(name=bn_name_base + '2b')

        self.conv2c = tf.keras.layers.Conv2D(filters3, (1, 1),
                                             kernel_initializer='he_normal', name=conv_name_base + '2c')
        self.bn2c = tf.keras.layers.BatchNormalization(name=bn_name_base + '2c')

        if self.downsampling:
            self.conv_shortcut = tf.keras.layers.Conv2D(filters3, (1, 1), strides=(stride, stride),
                                                        kernel_initializer='he_normal', name=conv_name_base + '1')
            self.bn_shortcut = tf.keras.layers.BatchNormalization(name=bn_name_base + '1')
...
# resnet 50 [3,4,6,3] 101:[3,4,23,3]
class ResNet(tf.keras.Model):
    ...
    def __init__(self, depth, **kwargs):
        ...
        self.depth = depth
        # stage 1
        self.padding = tf.keras.layers.ZeroPadding2D((3, 3))
